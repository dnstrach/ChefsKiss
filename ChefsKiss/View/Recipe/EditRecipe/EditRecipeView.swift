...
/*
 struct EditRecipeView: View {
   ...
   //  @StateObject private var viewModel = EditRecipeViewModel()
     @Bindable var viewModel: EditRecipeViewModel
         ...
         NavigationStack {
             Form {
                 Section {
                     ...
                     TextField("Summary", text: $viewModel.recipe.summary, axis: .vertical)
                 }
                 ...
                 Section {
                     ...
                     HStack {
                         ...
                         EditImagePickerView(
                             imageState: viewModel.imageState, recipe: viewModel.recipe
                         )
                 ...
                 Section {
                     Stepper("Serves \(viewModel.recipe.servings.formatted())", value: $viewModel.recipe.servings, in: 1...100, step: 0.5)
                 }
                 
                 Section("Prep Time") {
                     HStack {
                         Picker("Hours", selection: $viewModel.recipe.prepHrTime) {
                             ForEach(viewModel.prepHrRange, id: \.self) { hour in
                                 Text("\(hour)")
                             }
                         }
                         
                         Picker("Minutes", selection: $viewModel.recipe.prepMinTime) {
                             ForEach(viewModel.prepMinRange, id: \.self) { minute in
                                 Text("\(minute)")
                             }
                         }
                 ...
                 Section("Cook Time") {
                     HStack {
                         Picker("Hours", selection: $viewModel.recipe.cookHrTime) {
                             ForEach(viewModel.cookHrRange, id: \.self) { hour in
                                 Text("\(hour)")
                             }
                         }
                         
                         Picker("Minutes", selection: $viewModel.recipe.cookMinTime) {
                             ForEach(viewModel.cookMinRange, id: \.self) { minute in
                                 Text("\(minute)")
                             }
                         }
                 ...
                 Section("Instructions") {
                     List {
                         ...
                         ForEach(viewModel.sortedInstructions, id: \.id) { step in
                             Text("\(step.index + 1). \(step.step)")
                                 .swipeActions {
                                     Button(role: .destructive) {
                     ...
                     VStack {
                         ...
                         Button("Add Step", systemImage: "plus.circle") {
                             print(viewModel.recipe.instructions.count)
                             viewModel.addStep()
                         }
                         .disabled(viewModel.disableStep)
                     }
 ...
 #Preview {
     do {
         ...
         let viewModel = EditRecipeViewModel(recipe: previewer.recipe)
         
         return EditRecipeView(viewModel: viewModel)
             .modelContainer(previewer.container)
     } catch {
         fatalError("Failed to create preview container.")
     }
